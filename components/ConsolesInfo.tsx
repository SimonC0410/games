...
async function deleteConsole(formData: FormData) {
    ...
    if (gamesCount > 0) {
        redirect(`/consoles?error=${id}`)  // redirige con el id de la consola que falló
    }

    if (consoleItem.image && consoleItem.image !== "no-image.png") {
        const imagePath = path.join(process.cwd(), "public", "img", consoleItem.image)
        try {
...
export default async function ConsolesInfo({
    ...
    searchParams: { error?: string }  // 👈 recibir el error
}) {
    const consoles = await prisma.console.findMany()
    const errorConsoleId = searchParams?.error ? Number(searchParams.error) : null
    const errorConsole = errorConsoleId
        ? consoles.find((c) => c.id === errorConsoleId)
        : null
                    ...
                    <div key={console.id} className="card bg-black-100 w-96 h-[420px] shadow-sm flex flex-col border-2 border-purple-400">
                        ...
                        <div className="card-body flex flex-col justify-between bg-black/40 text-white h-150px">
                            ...
                            <div className="card-actions justbtn-outlineify-end flex">
                                ...
                                <label htmlFor={`delete_modal_${console.id}`} className="btn btn-outline btn-error">
                                    Delete
                                </label>
                                <input type="checkbox" id={`delete_modal_${console.id}`} className="modal-toggle" />
                                <div className="modal" role="dialog">
                                    <div className="modal-box bg-black border-2 border-purple-400">
                                        <h3 className="text-lg font-bold text-error">Delete Console</h3>
                                        <p className="py-4">
                                            Are you sure you want to delete
                                            <span className="font-bold"> {console.name}</span>?
                                        </p>
                                        <div className="modal-action">
                                            <label htmlFor={`delete_modal_${console.id}`} className="btn">Cancel</label>
                                            <form action={deleteConsole}>
                                                <input type="hidden" name="id" value={console.id} />
                                                <button type="submit" className="btn btn-error">Delete Console</button>
                                            </form>
                                        </div>
                                    </div>
                                </div>
